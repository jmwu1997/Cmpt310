...
class CSP(search.Problem):
    ...
    def nconflicts(self, var, val, assignment):
        ...
        self.conflict = count(conflict(v) for v in self.neighbors[var])
...
def min_conflicts(csp, max_steps=100000):
    ...
    for i in range(max_steps):
        ...
        if not conflicted:
            return current,reassign
        var = random.choice(conflicted)
        val = min_conflicts_value(csp, var, current)
        csp.assign(var, val, current)
        reassign+=1
    return None,reassign

def solve(graphs):
    assigns= 0
    conflict=0
    start_time = time.time()
    #At max 105 colors for 105 teams, one player each color
    for i in range(105):
        ...
        #print(AC3(csp))
        #using min_conflicts
        solution,reassign = min_conflicts(csp,5000)
        #total up unassigned and assigns
        assigns = assigns+csp.nassigns
        conflict = conflict+csp.conflict
        ...
        if solution != None and check_teams(graphs,solution):
            ...
            return solution, i+1, assigns, conflict , elasped_time-start_time,reassign
...
def run_q4():
    ...
    for i in range (5):
        ...
        for i in range(6):
                print('Graph: %s__________________________________' % (i+1))
                print(graphs[i])
                solution, number_of_teams ,assigns,conflict,time,reassign = solve(graphs[i])
                print('')
                print('Testing Graph %s with p: 0.%s and n = 105' % (i+1,i+1))
                ...
                print('4: the count of the number of conflicts: ',conflict)
                print('5: the count of the number of reassign: ',reassign)
                print('6: Max, Min, Average number in a team: ',get_stats(solution))
